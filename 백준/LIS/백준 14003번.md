...
`prevIndexOfLIS[i]` : `길이가 k인 LIS[i]를 만들 때, 참조하는 길이가 k-1인 LIS[m]이 있을 때, LIS[m]의 가장 마지막 원소의 A[m]의 인덱스 값 m`
...
5. i = 5일 때
   ...
   minNumOfLISLength[1]: 길이 1의 LIS를 만드는 A[i]의 값 중 최솟값은 2로 갱신됩니다.
